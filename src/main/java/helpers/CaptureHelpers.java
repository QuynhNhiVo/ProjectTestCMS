...
public class CaptureHelpers {
    ...
    public static void takeScreenShot(String screenshotName){
        try {
            ...
            File theDir = new File(SystemHelpers.getCurrentDirectory() + PropertiesHelpers.getValue("SCREENSHOT_PATH"));
            if (!theDir.exists()) {
                theDir.mkdirs();
            }
            // Chổ này đặt tên thì truyền biến "screenName" gán cho tên File chụp màn hình
            FileHandler.copy(source, new File(SystemHelpers.getCurrentDirectory() + PropertiesHelpers.getValue("SCREENSHOT_PATH") + File.separator + screenshotName + "_" + dateFormat.format(new Date()) + ".png"));
